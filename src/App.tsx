...
// Вспомогательный компонент для карточки продукта (с логикой иконок и вложенных полей)
    ...
    <AnimatePresence>
      {active && children && (
        <motion.div
          ...
          onClick={(e) => e.stopPropagation()} // Предотвращает выключение карточки при клике на поле ввода
        >
          {children}
        </motion.div>
      )}
...
export default function App() {
  ...
  const results = useMemo(() => {
    ...
    // 1. Прибыль сейчас (Current Profit)
    ...
    // 3. Расчет Smart-прибыли
    // «Буст» среднего чека и количества чеков для 40% клиентов
    const hasBoost = p1 || p2 || p3 || p4 || p8;
    const nAvg = hasBoost ? (avg * 0.6 + avg * 1.16 * 0.4) : avg; // +16% рост чека для 40%
    const nCh = p2 ? (totalChecks * 0.6 + totalChecks * 1.25 * 0.4) : totalChecks; // +25% рост чеков для 40%
    
    // Прибыль от возвращающихся клиентов (ret) и лояльности (loy)
    const ret = (p1 || p2 || p3 || p4 || p5 || p7 || p8) ? (nCh * 0.2 * nAvg) : 0; // 20% возвращающихся клиентов
    const loy = p5 ? (nCh * 0.2 * nAvg * 0.3) : 0; // Дополнительная прибыль от использования баллов лояльности

    // Smart profit
    let smartProfit = ((nCh * nAvg) + ret + loy) * marg - finalCost;
    if (!p3) smartProfit -= ((nCh * nAvg) * 0.3 * aggr); // Вычитаем комиссию агрегатора, если нет SR Delivery
  ...
  // Помощник для карточки параметра
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] p-4 md:p-12 font-sans">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Хедер Freedom Bank (как на твоих макетах) */}
        ...
        <div className="flex flex-col lg:flex-row gap-12 items-start">
          ...
          {/* ИТОГОВАЯ ПАНЕЛЬ (Sticky Sidebar) */}
          <div className="w-full lg:w-[420px] lg:sticky lg:top-12 mt-12 lg:mt-0">
            <div className="bg-white p-10 rounded-[48px] border border-slate-100 shadow-2xl shadow-slate-200/50">
              ...
              <div className="space-y-6">
                {/* Прибыль сейчас (Current Profit) - серый */}
                ...
                {/* С Smart Restaurant - зеленый */}
                <div className="bg-green-500 p-10 rounded-[32px] text-center text-white shadow-xl shadow-green-200">
                  <div className="text-[10px] font-bold opacity-80 uppercase mb-1.5 tracking-wider tracking-wider">С Smart Restaurant</div>
                ...
                {/* Выгода внедрения - светло-зеленый */}
